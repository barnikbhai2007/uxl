...
// --- Auth Mock ---
export type User = { uid: string; email: string | null; isAnonymous?: boolean; emailVerified?: boolean; tenantId?: string, providerData?: any[] };
...
export async function signInAnonymously(auth: any) {
  ...
  if (error) throw error;
  return { user: { uid: data.user?.id || 'anon', email: null, isAnonymous: true } };
}
...
export function onAuthStateChanged(auth: any, callback: (user: User | null) => void) {
  const { data } = supabase.auth.onAuthStateChange((event, session) => {
    if (session?.user) {
      const u: User = { uid: session.user.id, email: session.user.email || null, isAnonymous: session.user.is_anonymous, providerData: [] };
      auth.currentUser = u;
      callback(u);
    } else {
...
// --- Firestore Mock ---
export function getFirestore(app: any, id?: string) { return 'db'; }
...
class Query {
  constructor(public collectionName: string, public filters: any[] = [], public orderBys: any[] = [], public qlimit?: number) {}
}
...
export function doc(db: any, colName: string, id?: string) {
  ...
  if (colName && id === undefined && (colName as any) instanceof CollRef === false) {
  ...
  return new DocRef(colName as string, id || crypto.randomUUID());
}
...
export function query(coll: CollRef | Query, ...constraints: any[]) {
  const q = new Query(coll.collectionName, 
...
export async function getDoc(ref: DocRef) {
  const { data, error } = await supabase.from('documents').select('*').eq('collection', ref.collectionName).eq('id', ref.id).single();
  return {
    exists: () => !!data,
    id: ref.id,
...
export async function getDocs(ref: CollRef | Query) {
  let sb = supabase.from('documents').select('*').eq('collection', ref.collectionName);
...
export async function setDoc(ref: DocRef, data: any, options: any = {}) {
  ...
  if (options && options.merge) {
    const { data: existing } = await supabase.from('documents').select('data').eq('collection', ref.collectionName).eq('id', ref.id).single();
    if (existing) {
       await supabase.from('documents').update({ data: { ...existing.data, ...finalData } }).eq('collection', ref.collectionName).eq('id', ref.id);
    } else {
       await supabase.from('documents').insert({ id: ref.id, collection: ref.collectionName, data: finalData });
    }
  } else {
    await supabase.from('documents').upsert({ id: ref.id, collection: ref.collectionName, data: finalData }, { onConflict: 'collection,id' });
  }
}
...
export async function updateDoc(ref: DocRef, data: any) {
  ...
  if (existing) {
     ...
     await supabase.from('documents').update({ data: nextData }).eq('collection', ref.collectionName).eq('id', ref.id);
  }
}

export async function deleteDoc(ref: DocRef) {
  await supabase.from('documents').delete().eq('collection', ref.collectionName).eq('id', ref.id);
}
...
export class Timestamp {
   static now() { return { toMillis: () => Date.now(), toDate: () => new Date() }; }
}
export function increment(val: number) { return val; }
export function arrayUnion(...vals: any[]) { return vals; }

export function writeBatch(db: any) {
  const operations: any[] = [];
  return {
    set: (ref: DocRef, data: any) => operations.push({ type: 'set', ref, data }),
    update: (ref: DocRef, data: any) => operations.push({ type: 'update', ref, data }),
